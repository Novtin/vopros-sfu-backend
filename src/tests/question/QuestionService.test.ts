...
describe('QuestionService', () => {
  ...
  describe('rate', () => {
    it('should rate QuestionModel', async () => {
      ...
      const ratedQuestion = await questionService.rate(rateDto);
    ...
    it('should throw ConflictException if this rating is exists', async () => {
      ...
      await questionService.rate(rateDto);

      await expect(questionService.rate(rateDto)).rejects.toThrow(
        ConflictException,
      );
    });
  ...
  describe('deleteRate', () => {
    it('should delete rating QuestionModel', async () => {
      ...
      await questionService.rate(dto);
      await questionService.deleteRate(dto);
  ...
  describe('setFavorite', () => {
    it('should set favorite QuestionModel', async () => {
      ...
      await questionService.setFavorite(favoriteDto);
    ...
    it('should throw ConflictException if question is already in favorite', async () => {
      ...
      await expect(questionService.setFavorite(favoriteDto)).rejects.toThrow(
        ConflictException,
      );
    });
  ...
  describe('deleteFavorite', () => {
    it('should delete favorite QuestionModel', async () => {
      ...
      await questionService.setFavorite(dto);
      await questionService.deleteFavorite(dto);
      const updatedUser = await userService.getOneBy({ id: user.id });
    ...
    it('should throw NotFoundException if question is not exists', async () => {
      await expect(
        questionService.deleteFavorite({
          questionId: 0,
          userId: user.id,
        }),
      ).rejects.toThrow(NotFoundException);
